...
export class LocationComponent implements OnInit {
  ...
  public searchField = new FormControl('LocationID');
  ...
  loadCSV() {
    this.http.get('assets/data/Location.csv', { responseType: 'text' }).subscribe(data => {
      const lines = data.split('\n');
      this.locationData = lines.slice(1)
        .map(line => line.trim())
        .filter(line => line)
        .map(line => {
  ...
  performSearch(term: string) {
    const field = this.searchField.value as keyof ILocation;
    const lowerTerm = term.toLowerCase();

    this.searchResults = this.locationData.filter(p =>
  ...
  exportToCSV() {
    ...
    link.href = url;
    link.setAttribute('download', 'Location_Search_Results.csv');
